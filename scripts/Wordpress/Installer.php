...
class Installer {
  ...
  public static function symlinkWPContent(Event $event) {
    ...
    $wp_core_content_folder = "{$root}/htdocs/wp/wp-content";
    $wp_content_folder = "{$root}/web/app";
    ...
    if (!is_link($wp_core_content_folder)) {
      if(file_exists($wp_core_content_folder)) {
        ...
        if (self::isWindows()) {
          ...
          symlink("../content", $wp_core_content_folder);
          $io->write("Removed wp-content from core and symlinked it to ../content");
        }
      }
    }
  }
